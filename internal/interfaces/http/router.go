...
func NewRouter(svc *app.Services) *gin.Engine {
	...
	vacancyHandler := NewVacancyHandler(svc.Vacancy, svc.CompanyHR, svc.VacancySearch, svc.VacancyApplication, svc.CandidateSearch, userHandler)
	...
	v1 := router.Group("/api/v1")
	{
		...
		vacancies := v1.Group("/vacancies")
		{
			...
			vacancies.GET("/:id/matches", JWTMiddleware(svc.JWTSecret), CasbinMiddleware(svc.CasbinEnforcer, "vacancies", "read"), vacancyHandler.GetMatches)
		...
		hrMiniapp.Use(TelegramHRAuthMiddleware(svc.TelegramHRBotToken, svc.CompanyHR))
		{
			...
			hrMiniapp.GET("/vacancies/:id/matches", vacancyHandler.GetMatches)
